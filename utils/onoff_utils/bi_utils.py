...
class OnOff_LlamaDecoderLayer(nn.Module):
    def __init__(self, original_decoder_layer):
        ...
        self.cal_bi = True
        #self.importance = [0 for _ in self.model.model.layers] #如何修改为其他数
        self.importance = 0
    ...
    def forward(
        ...
        if self.cal_bi is True:
            ...
            importance = block_importance(input, output)
...
def global_influence(input_hidden_state: torch.Tensor,
    output_hidden_state: torch.Tensor,
    angular=False):
